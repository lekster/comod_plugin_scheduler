...
class TaskRun extends ActiveRecord implements TaskRunInterface
{
    ...
    {
        $db = (new Query())
            ->select('cron_task_runs.*, cron_task.command')
            ->from(self::tableName())
            ->join('LEFT JOIN', 'cron_task', 'cron_task.task_id = cron_task_runs.task_id')
            ->orderBy('cron_task_runs.task_run_id desc')
            ->limit($count);
        if ($task_id) {
            $db->where('cron_task_runs.task_id=:task_id', array(':task_id' => $task_id));
        }
